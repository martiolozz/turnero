...
referenciaProximosTurnos.orderByChild("consultorio").equalTo("302").on('value', gotProximos, errData);

// Creo la referencia al "nodo" consultorios en la base de datos
var refConsultorios = database.ref(stringFecha + '/consultorios');
refConsultorios.orderByChild("consultorio").equalTo("302").on('value', gotProfesional, errData);

// Creo la referencia al "nodo" turnos-activos
var referenciaTurnoActivo = database.ref(stringFecha + '/turnos-activos');
referenciaTurnoActivo.orderByChild("consultorio").equalTo("302").on('value', gotActivo, errData);

// Creo la referencia al "nodo" chat
var referenciaChat = database.ref(stringFecha + '/chat');
referenciaChat.orderByChild("receptor").equalTo("Consultorio 302:").on('value', gotChat, errData);
...
function gotProximos(data) {
    ...
    for (let i = 0; i < keys.length; i++) {
        ...
        var categoria = turnos[k].categoria;

        const nuevoTurno = document.createElement('div')
        nuevoTurno.classList.add('proximo-turno')
        if (categoria == "Teleconsulta") {
            nuevoTurno.classList.add('teleconsulta')
        }
        if (categoria == "Consulta Externa") {
            nuevoTurno.classList.add('consulta-externa')
        }
        if (categoria == "Consulta Interna") {
            nuevoTurno.classList.add('consulta-interna')
        }
        if (categoria == "Ambulatorio") {
            nuevoTurno.classList.add('ambulatorio')
        }
