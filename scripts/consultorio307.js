...
referenciaProximosTurnos.orderByChild("consultorio").equalTo("307").on('value', gotProximos, errData);

// Creo la referencia al "nodo" consultorios en la base de datos
var refConsultorios = database.ref(stringFecha + '/consultorios');
refConsultorios.orderByChild("consultorio").equalTo("307").on('value', gotProfesional, errData);

// Creo la referencia al "nodo" turnos-activos
var referenciaTurnoActivo = database.ref(stringFecha + '/turnos-activos');
referenciaTurnoActivo.orderByChild("consultorio").equalTo("307").on('value', gotActivo, errData);

// Creo la referencia al "nodo" chat
var referenciaChat = database.ref(stringFecha + '/chat');
referenciaChat.orderByChild("receptor").equalTo("Consultorio 307:").on('value', gotChat, errData);
